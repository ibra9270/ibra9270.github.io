app.controller("MainController", ["$scope", "uiGmapGoogleMapApi", "ColorService", 'ScrollService',
    function($scope, uiGmapGoogleMapApi, ColorService, ScrollService) {
        var birthdate = moment("1991-02-28 15:59").startOf('minute');
        ...
        this.ivesimages = [];
        for (let i = 0; i < 1; i++) {
            this.ivesimages.push("res/images/ibra" + i + ".png");
        };

        this.currentIbra = Math.floor(Math.random() * 1)
        this.changeIbraImage = () => {
            if (!$scope.$$phase) {
                $scope.$apply(() => { this.currentIbra = (this.currentIbra + 1) % 1; });
            } else {
                this.currentIbra = (this.currentIbra + 1) % 1;
            }
        }
